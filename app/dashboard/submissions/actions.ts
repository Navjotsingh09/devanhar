...
export async function capturePadelPayment(registrationId: string) {
  ...
  if (reg.captain_email) {
    sendPadelRegistrationApprovedEmail({ to: reg.captain_email, firstName: reg.captain_first_name || 'Player', teamName: reg.player2_first_name ? (reg.captain_first_name || 'Player') + ' & ' + reg.player2_first_name : undefined })
      .catch((err: unknown) => console.error('[Padel Capture] Approval email failed (non-blocking):', err))
  }
  revalidatePath('/dashboard/submissions')
}

export async function cancelPadelPayment(registrationId: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (user == null) throw new Error('Unauthorized')
  ...
  if (reg.captain_email) {
    sendPadelRegistrationDeclinedEmail({ to: reg.captain_email, firstName: reg.captain_first_name || 'Player', teamName: reg.player2_first_name ? (reg.captain_first_name || 'Player') + ' & ' + reg.player2_first_name : undefined })
      .catch((err: unknown) => console.error('[Padel Decline] Decline email failed (non-blocking):', err))
  }
  revalidatePath('/dashboard/submissions')
}
...
// ---------------------------------------------------------------------------
// SPN submissions (Join / Advisor / Grad Award / Event) — mirrors vidyala.
// Status -> 'approved' / 'declined' + a Resend email to the applicant.
// ---------------------------------------------------------------------------
export async function approveSpnSubmission(submissionId: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Unauthorized')
  ...
  const { data: sub } = await supabase
    ...
    .select('id, first_name, last_name, email, status, submission_type')
    .eq('id', submissionId)
    .single()
  ...
  await supabase.from('activity_log').insert({
    admin_id: user.id,
    action: "Approved SPN submission: " + (sub.first_name || '') + ' ' + (sub.last_name || ''),
    entity_type: 'spn_submission',
    entity_id: String(submissionId),
  })

  if (sub.email) {
    sendSpnApprovalEmail({ to: sub.email, firstName: sub.first_name || 'there', submissionType: sub.submission_type || 'join' })
      .catch(err => console.error('[SPN] Approval email failed (non-blocking):', err))
  }

  revalidatePath('/dashboard/submissions')
}

export async function declineSpnSubmission(submissionId: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Unauthorized')
  ...
  const { data: sub } = await supabase
    ...
    .single()
  if (!sub) throw new Error('Submission not found')
  if (sub.status === 'declined') throw new Error('Already declined')
  ...
  await supabase.from('activity_log').insert({
    admin_id: user.id,
    action: "Declined SPN submission: " + (sub.first_name || '') + ' ' + (sub.last_name || ''),
    entity_type: 'spn_submission',
    entity_id: String(submissionId),
  })

  if (sub.email) {
    sendSpnDeclineEmail({ to: sub.email, firstName: sub.first_name || 'there', submissionType: sub.submission_type || 'join' })
      .catch(err => console.error('[SPN] Decline email failed (non-blocking):', err))
  }
