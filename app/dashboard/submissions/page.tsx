...
async function getSubmissions() {
  ...
  const [initiativesRes, submissionsRes, campAppsRes, vidyalaAppsRes, padelRegsRes, spnSubsRes, interestRes, rootsRes, wolfRunFundraisersRes, familyInitiativeRes, recoveredPaymentsRes] = await Promise.all([
    ...
    supabase.from('recovery_payment_ledger').select('id, source, occurred_at, amount, currency, payment_status, description, customer_name, customer_email, customer_phone, payment_reference, linked_record_type, linked_record_id').order('occurred_at', { ascending: false }).limit(5000),
  ])
  ...
  const recoveredSubmissions: RecoveredSubmission[] = recoveredPayments.map((payment: Record<string, unknown>) => ({
    ...
    status: String(payment.payment_status ?? 'recorded'),
    details: [
      payment.description,
      payment.amount == null ? null : `Amount: ${String(payment.currency ?? 'GBP').toUpperCase()} ${Number(payment.amount).toFixed(2)}`,
      payment.customer_phone ? `Phone: ${String(payment.customer_phone)}` : null,
      payment.linked_record_type && payment.linked_record_id ? `Original link: ${String(payment.linked_record_type)} ${String(payment.linked_record_id)}` : null,
      payment.payment_reference ? `Reference: ${String(payment.payment_reference)}` : null,
    ].filter(Boolean).join(' | '),
    created_at: String(payment.occurred_at ?? new Date().toISOString()),
  }))
...
export default async function SubmissionsPage() {
  const [{ initiatives, submissions, webinarSignups, otherSubmissions, recoveredSubmissions }, recentActivity] = await Promise.all([
    getSubmissions(),
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="h-auto bg-transparent p-0 flex flex-wrap gap-3 mb-2">
          ...
          <TabsTrigger value="__recovered" className="group flex-col items-start gap-0.5 h-auto px-4 py-3 min-w-[90px] rounded-xl border border-border bg-card text-left shadow-sm transition-all data-[state=active]:border-primary data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md hover:border-primary/50 hover:bg-muted/60">
            <span className="text-lg font-bold leading-none">{recoveredSubmissions.length}</span>
            <span className="text-[11px] font-medium leading-none opacity-70">Recovered Records</span>
          </TabsTrigger>
          <TabsTrigger
            value="__webinar"
            className="group flex-col items-start gap-0.5 h-auto px-4 py-3 min-w-[90px] rounded-xl border border-border bg-card text-left shadow-sm transition-all data-[state=active]:border-primary data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md hover:border-primary/50 hover:bg-muted/60"
