...
const bucketName = process.env.SUPABASE_PADEL_UPLOAD_BUCKET || process.env.SUPABASE_CAMP_UPLOAD_BUCKET || "camp-applications"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const initiativeSlug = String(formData.get("initiative_slug") || "sikh-padel-association")
    ...
    if (error) {
      console.error("[Padel ID Upload] Supabase storage error:", error)
    ...
    console.error("[Padel ID Upload] Error:", error)
