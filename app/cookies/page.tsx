            ...
            {sections.map((s, i) => (
              <div key={i}>
                ...
                {s.blocks.map((b, j) => {
                  ...
                  if (b.t === "table")
                    return (
                      <div key={j} className="not-prose my-6">
                        <p className="text-sm font-semibold mb-2">{b.caption}</p>
                        <div className="overflow-x-auto rounded-lg border border-border/60">
                          <table className="w-full text-sm border-collapse">
                            <thead>
                              <tr className="bg-muted/40">
                                {tableHead.map((th, n) => (
                                  <th key={n} className="border-b border-border/60 px-3 py-2 text-left font-semibold">
                                    {th}
                                  </th>
                                ))}
                              </tr>
                            ...
                            <tbody>
                              {b.rows.map((row, r) => (
                                <tr key={r} className="odd:bg-muted/20">
                                  {row.map((cell, n) => (
                                    <td key={n} className="border-b border-border/40 px-3 py-2 align-top">
                                      {cell}
                                    </td>
                                  ))}
                                </tr>
